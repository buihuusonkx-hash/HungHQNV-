      ...
      <header className="max-w-4xl mx-auto text-center mb-12">
        ...
        >
          ...
          <button
            ...
            className="mt-4 px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-violet-500 text-white rounded-xl font-bold text-sm flex items-center mx-auto hover:from-indigo-600 hover:to-violet-600 transition-all shadow-lg shadow-indigo-200 hover:scale-105"
          >
    ...
    >
      ...
      {isGenerated && cauHois.length > 0 && (
        ...
        >
          ...
          {/* ═══ BẢNG ĐÁP ÁN ════════════════════════════════════════════ */}
          <div className="bg-white rounded-3xl border-2 border-indigo-200 shadow-lg overflow-hidden mt-6">
            <div className="bg-indigo-700 px-8 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-white font-black text-base">ĐÁP ÁN</h3>
                <p className="text-indigo-200 text-xs mt-0.5">Bảng đáp án đầy đủ cho đề thi</p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              {nlcQuestions.length > 0 && (
                <div>
                  <h4 className="text-sm font-black text-emerald-700 mb-3 uppercase tracking-wide">Phần I. Trắc nghiệm nhiều phương án ({nlcQuestions.length} câu — mỗi câu 0,25đ)</h4>
                  <div className="overflow-x-auto rounded-xl border border-emerald-200">
                    <table className="w-full border-collapse text-sm">
                      <thead>
                        <tr className="bg-emerald-600 text-white text-center font-black text-xs">
                          {nlcQuestions.map((_, i) => (
                            <th key={i} className="border border-emerald-500 px-3 py-2 min-w-[44px]">Câu {i + 1}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        <tr className="text-center font-black text-emerald-800 bg-emerald-50">
                          {nlcQuestions.map((q, i) => (
                            <td key={i} className="border border-emerald-200 px-3 py-2.5 text-base">{q.dapAn}</td>
                          ))}
                        </tr>
                      </tbody>
                    </table>
              ...
              {dsQuestions.length > 0 && (
                <div>
                  <h4 className="text-sm font-black text-amber-700 mb-1 uppercase tracking-wide">Phần II. Trắc nghiệm Đúng / Sai ({dsQuestions.length} câu — mỗi câu 4 ý)</h4>
                  <p className="text-[11px] text-amber-600 mb-3 italic">1 ý đúng: 0,1đ · 2 ý: 0,25đ · 3 ý: 0,5đ · 4 ý: 1,0đ</p>
                  <div className="overflow-x-auto rounded-xl border border-amber-200">
                    <table className="w-full border-collapse text-sm">
                      <thead>
                        <tr className="bg-amber-500 text-white text-center font-black text-xs">
                          <th className="border border-amber-400 px-4 py-2 w-20">Câu</th>
                          <th className="border border-amber-400 px-4 py-2">a)</th>
                          <th className="border border-amber-400 px-4 py-2">b)</th>
                          <th className="border border-amber-400 px-4 py-2">c)</th>
                          <th className="border border-amber-400 px-4 py-2">d)</th>
                        </tr>
                      </thead>
                      <tbody>
                        {dsQuestions.map((q, i) => {
                          const daParts = (q.dapAn || '').split(/\s+/);
                          return (
                            <tr key={i} className="text-center font-bold bg-amber-50 hover:bg-amber-100 transition-colors">
                              <td className="border border-amber-200 px-3 py-2.5 font-black text-amber-800">Câu {i + 1}</td>
                              {[0, 1, 2, 3].map(k => {
                                const val = (daParts[k] || '...').toUpperCase();
                                const isD = val === 'Đ' || val === 'D';
                                return (
                                  <td key={k} className={`border border-amber-200 px-3 py-2.5 text-base font-black ${isD ? 'text-emerald-700 bg-emerald-50' : 'text-rose-600 bg-rose-50'}`}>
                                    {val === 'Đ' || val === 'D' ? 'Đ' : val === 'S' ? 'S' : val}
                                  </td>
                                );
                              })}
                            </tr>
              ...
              {tlnQuestions.length > 0 && (
                <div>
                  <h4 className="text-sm font-black text-rose-700 mb-3 uppercase tracking-wide">Phần III. Trả lời ngắn ({tlnQuestions.length} câu — mỗi câu 0,5đ)</h4>
                  <div className="overflow-x-auto rounded-xl border border-rose-200">
                    <table className="w-full border-collapse text-sm">
                      <thead>
                        <tr className="bg-rose-600 text-white text-center font-black text-xs">
                          {tlnQuestions.map((_, i) => (
                            <th key={i} className="border border-rose-500 px-4 py-2 min-w-[60px]">Câu {i + 1}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        <tr className="text-center font-black text-rose-800 bg-rose-50">
                          {tlnQuestions.map((q, i) => (
                            <td key={i} className="border border-rose-200 px-3 py-2.5 text-base">{q.dapAn}</td>
                          ))}
                        </tr>
                      </tbody>
                    </table>
